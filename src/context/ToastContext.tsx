...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  // demo trigger after 8s
  useEffect(() => {
    const id = setTimeout(() => {
      push({
        tone: "danger",
        title: "GSR spike — Anak 001",
        description: "8.3 μS — Threshold exceeded. Haptic nudge dispatched.",
      });
    }, 8000);
      ...
      <div className="fixed bottom-5 right-5 z-[100] flex flex-col gap-3 w-[320px] pointer-events-none">
        {toasts.map((t) => {
          ...
          return (
            <div
              key={t.id}
              className="glass-card toast-in p-3 pl-4 pointer-events-auto flex gap-3"
